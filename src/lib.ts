...
export function getPropertyIntInit(name: string, defaultValue: number) {
  const str = getProperty(name);
  if (str === '') {
    setProperty(name, defaultValue.toString());
...
export function fightSausageIfGuaranteed() {
  if (sausageFightGuaranteed()) {
    ...
    print(`grabbing Kramco`);
    const currentOffhand = equippedItem($slot`off-hand`);
    equip($item`Kramco Sausage-o-Matic™`);
    print('Adventure in Noob Cave to fight a sausage');
    adventureHere($location`Noob Cave`, $familiar`Cat Burglar`);
...
export function fuelAsdon(goal: number) {
  ...
  if (startingFuel > goal) {print('We have enough fuel'); return startingFuel;}
...
export function ensureAsdonEffect(ef: Effect, turns:number = 30) {
  while (haveEffect(ef) < turns) {

    const fuelNeeded = Math.ceil((turns-haveEffect(ef))/30) * 37;
    print(`Have: ${haveEffect(ef)} Need: ${turns} FuelNeeded: ${fuelNeeded}`);
      fuelAsdon(fuelNeeded);
...
export function lastAdventureText(): string {
  ...
  return log.lastIndexOf('['+myTurncount()+1+']') > 0 ? log.substring(log.lastIndexOf('['+myTurncount()+1+']')) : log.substring(log.lastIndexOf('['+myTurncount()+']'));
}
