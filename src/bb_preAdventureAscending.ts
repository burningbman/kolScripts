import { abort, availableAmount, canAdventure, combatRateModifier, creatableAmount, equip, equippedItem, familiarEquippedEquipment, haveEquipped, Item, itemType, Location, myFamiliar, myLevel, myLocation, myMaxmp, myMp, myTurncount, print, totalTurnsPlayed, useFamiliar, userConfirm } from "kolmafia";
import { $effect, $familiar, $item, $location, $locations, $slot, get, have } from "libram";
...
function handleAccessory() {
    ...
    if (myLocation() === $location`The Batrat and Ratbat Burrow` && !haveEquipped($item`Retrospecs`)) error('equip retrospecs');
}

const WEAPON_LOCS = new Map<Item, Location[]>();
