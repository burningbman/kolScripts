import { autosell, availableAmount, cliExecute, putShop, use } from "kolmafia";
import { $item, set } from "libram";

const MALL_MIN_ITEMS = [
  $item`cyan seashell`,
  $item`gray seashell`,
  $item`green seashell`,
  $item`magenta seashell`,
  $item`yellow seashell`,
  $item`kelp`,
];

export function main(): void {
  for (const item of MALL_MIN_ITEMS) {
    putShop(100, 0, item);
  }
  ...
  autosell(
    availableAmount($item`spearfish fishing spear`),
    $item`spearfish fishing spear`
  );
  autosell(
    availableAmount($item`lucky rabbitfish fin`),
    $item`lucky rabbitfish fin`
  );
  ...
  if (availableAmount($item`driftwood beach comb`) === 0)
